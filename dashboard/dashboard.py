...
@st.cache_data
def load_data():
    # Gunakan os.path agar aman di Local maupun Streamlit Cloud
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, "main_data.csv")
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### Filter Analisis")
    # FITUR INTERAKTIF: Memungkinkan pengguna memilih tahun
    selected_year = st.selectbox(
        "Pilih Tahun untuk Analisis Wanliu:", 
        options=sorted(main_df['year'].unique()), 
        index=3 # Default ke 2016
...
with col_m1:
    st.metric("Total Observasi", f"{len(main_df):,}")
    st.caption("⚠️ Menampilkan 50% sampel data untuk optimalisasi performa.")
with col_m2:
    avg_pm = main_df['PM2.5'].mean()
    st.metric("Avg PM2.5 (Keseluruhan)", f"{avg_pm:.2f} µg/m³")
with col_m3:
...
with c2:
    st.warning("⚠️ Tren Menurun Tidak Terdeteksi")
    st.write(f"- 2014/2015: **81.49 µg/m³**\n- 2015/2016: **97.57 µg/m³**")
    st.info("Terjadi kenaikan konsentrasi sebesar ~19.7%.")
...
# TAB UNTUK MENJAWAB PERTANYAAN 2 SECARA LENGKAP
tab1, tab2, tab3 = st.tabs(["Matriks Korelasi (MANDATORY)", "Tren Bulanan", "Distribusi & Kategori"])

with tab1:
    st.subheader("Matriks Korelasi PM2.5 vs Faktor Meteorologi")
    # Menampilkan Heatmap yang diminta Reviewer
    fig_corr, ax_corr = plt.subplots(figsize=(10, 8))
    # Filter kolom numerik saja untuk korelasi
    ...
    st.pyplot(fig_corr)
    st.write("**Insight:** Kecepatan angin (WSPM) menunjukkan korelasi negatif yang paling konsisten terhadap PM2.5.")

with tab2:
    st.subheader("Tren Bulanan: PM2.5 vs Kecepatan Angin (WSPM)")
    monthly_trend = wanliu_year.groupby('month').agg({'PM2.5':'mean', 'WSPM':'mean'}).reset_index()
    fig_trend, ax1 = plt.subplots(figsize=(12, 6))
    ax2 = ax1.twinx()
    sns.lineplot(data=monthly_trend, x='month', y='PM2.5', marker='o', color='red', label='PM2.5', ax=ax1)
    sns.lineplot(data=monthly_trend, x='month', y='WSPM', marker='s', color='blue', label='WSPM', ax=ax2)
    ax1.set_ylabel("PM2.5", color='red')
    ax2.set_ylabel("WSPM", color='blue')
    st.pyplot(fig_trend)
...
with tab3:
    ...
    with col_a:
        ...
        sns.boxplot(x='month', y='PM2.5', data=wanliu_year, palette='viridis', ax=ax_box)
        ax_box.axhline(150, color='red', linestyle='--')
        st.pyplot(fig_box)
    with col_b:
        def categorize(val):
            if val <= 35: return 'Baik'
        ...
        wanliu_year['category'] = wanliu_year['PM2.5'].apply(categorize)
        st.dataframe(wanliu_year['category'].value_counts())

st.caption("Copyright © 2026 | Nurul Ainil Fitri")
